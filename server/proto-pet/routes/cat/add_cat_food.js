...
router.post('/', function(req,res){
    ...
    var fullpath = path.join(__dirname,savedata.path)
      ...
      form.on('close',function(){
            ...
            db.cat_food.create(row).then(function(food_data){
                  //console.log(food_data);
                  catId=food_data.id;
                  savedata.ingredients.split(',').reduce(function(p, ing) {
                        return p.then(function() {
                              return db.cat_ingredients.create({ content_name: ing, catFoodId: catId });
                        });
                  }, Promise.resolve())
                  .then(function(){
                        ...
                        return p.then(function() {
                              if(ing.trim().length)
                              {
                                    return db.cat_health_consideration.create({ content_name: ing, catFoodId: catId });
                              }else{
                                    return null;
                              }
                        ...
                        .then(function(){
                              savedata.nutritionalOption.split(',').reduce(function(p, ing) {
                                    return p.then(function() {
                                          if(ing.trim().length){
                                                return db.cat_nutritional_option.create({ content_name: ing, catFoodId: catId });
                                          }else{
                                                return null;
                                          }
                                    });
