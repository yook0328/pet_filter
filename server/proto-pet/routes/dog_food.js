...
router.get('/countFoodItems', function(req,res){
    db.dog_food.count().then(function(count){
        result = {
            count : count
        }
...
router.post('/testForItem', function(req,res){
...
router.post('/findFoodItems', function(req,res){
    ...
    .then(function(result){
        ...
        db.dog_guaranteed_analysis.findAll({
            where : {
                $or : query_GA
            }
        }).then(function(result){
            ...
            for(var i = 0; i < result.length; i++){
                ...
                "guaranteed_analysis_maxOrmin","dogFoodId"));
            }     
                 
            for(var i = 0; i < sendData.length; i++){
                sendData[i]["GA"] = GA_result.filter(function(value){
                    return value.dogFoodId == sendData[i].id
                })
            }
...
router.get('/loadImage/:filename', function(req,res){
    ...
    res.download('./upload/dogfood/' + filename, function(err){
        if (err) {
            console.log(err);
            res.status(400).send('Error');
        } else {
...
router.get('/deleteFoodItem/:foodId', function(req,res){
    ...
    var fullpath = path.join(__dirname,'../upload/dogfood/');


    db.dog_food.findById(foodId).then(function(data){
            fs.unlinkSync(fullpath+data.food_picture_name);
            var d02 = function(){
                return Promise.resolve(db.dog_ingredients.destroy({
                    where : {dogFoodId: foodId}
                }));
            }
            var d03 = function(){
                return Promise.resolve(db.dog_nutritional_option.destroy({
                    where : {dogFoodId: foodId}
                }))
            }
            var d04 = function(){
                return Promise.resolve(db.dog_health_consideration.destroy({
                        where : {dogFoodId: foodId}
                }));
            }
            var d05 = function(){
                return Promise.resolve(db.dog_guaranteed_analysis.destroy({
                    where : {dogFoodId: foodId}
                }));
            }
            Promise.all([, d02(), d03(), d04(), d05()]).then(function(result){
                return Promise.resolve(db.dog_food.destroy({
                    where : {id : foodId }
                }))
...
router.post('/loadList', function(req, res){
    ...
    db.dog_food.findAll({
        ...
        for(var i = 0; i < data_set.length; i++){
            ...
            promisGroup.push(db.dog_ingredients.findAll({where:{
                dogFoodId : data_set[i].id
            }}));
            promisGroup.push(db.dog_health_consideration.findAll({where:{
                dogFoodId : data_set[i].id
            }}));
            promisGroup.push(db.dog_nutritional_option.findAll({where:{
                dogFoodId : data_set[i].id
            }}));
            promisGroup.push(db.dog_guaranteed_analysis.findAll({where:{
                dogFoodId : data_set[i].id
            }}));
        }
...
router.get('/get_ingredients', function(req,res){
    ...
    db.dog_ingredients.aggregate('content_name', 'DISTINCT', { plain : false })
    .then(function(data){
        var result = '';
        for(var i = 0; i < data.length; i++){
            if(i != 0){
...
router.get('/get_brands', function(req,res){
    ...
    db.dog_food.aggregate('brand', 'DISTINCT', { plain : false })
    .then(function(data){
        var result = '';
        for(var i = 0; i < data.length; i++){
            if(i != 0){
...
router.post('/modifyFoodItems', function(req,res){
   
   db.dog_ingredients.findAll()
   .then(function(result){
       var data = modifyHelper(result);
       
       Object.keys(data).reduce( function(p, key){
...
router.post('/modifyCatFoodItems', function(req,res){
   
   db.cat_ingredients.findAll()
   .then(function(result){
       var data = modifyHelperForCat(result);
       console.log(data);
       Object.keys(data).reduce( function(p, key){
           return p.then(function() {
                var d = {
...
function modifyHelperForCat(_result){
    var result = {};

    for(var i = 0; i < _result.length; i++){
        if(!(_result[i].catFoodId in result)){
            result[_result[i].catFoodId] = _result[i].content_name;
        }else{
            result[_result[i].catFoodId] += ',' + _result[i].content_name;
        }
...
function modifyHelper(_result){
    var result = {};

    for(var i = 0; i < _result.length; i++){
        if(!(_result[i].dogFoodId in result)){
            result[_result[i].dogFoodId] = _result[i].content_name;
        }else{
            result[_result[i].dogFoodId] += ',' + _result[i].content_name;
        }
