...
router.get('/countFoodItems', function(req,res){
    ...
    db.dog_treats.count().then(function(count){
        result = {
            count : count
        }
...
router.get('/loadImage/:filename', function(req,res){
    ...
    res.download('./upload/dogtreats/' + filename, function(err){
        if (err) {
            console.log(err);
...
router.get('/deleteFoodItem/:foodId', function(req,res){
    ...
    var fullpath = path.join(__dirname,'../../../upload/dogtreats/');


    db.dog_treats.findById(foodId).then(function(data){
            fs.unlinkSync(fullpath+data.food_picture_name);
            var d02 = function(){
                return Promise.resolve(db.dog_treats_ingredients.destroy({
                    where : {dogTreatId: foodId}
                }));
            }
            var d05 = function(){
            ...
            Promise.all([, d02(),  d05()]).then(function(result){
                return Promise.resolve(db.dog_treats.destroy({
                    where : {id : foodId }
                }))
...
router.post('/findFoodItems', function(req,res){
    ...
    .then(function(result){
        ...
        db.dog_treats_guaranteed_analysis.findAll({
            where : {
                $or : query_GA
            }
        }).then(function(result){
...
router.post('/loadList', function(req, res){
    ...
    db.dog_treats.findAll({
        ...
        promisGroup.push(db.dog_treats_ingredients.findAll({
            where : {
                $or : idList
            }         
        }));
        promisGroup.push(db.dog_treats_guaranteed_analysis.findAll({
            where : {
                $or : idList
            }
        }));
...
function getIngredientsAndGA(result, data){
    ...
    for(var i = 0; i < send_data.length; i++){
        send_data[i].ingredients = getIngredients(result[0].filter(function(value){
            return value.dogTreatId == send_data[i].id
        }))
...
router.get('/get_brands', function(req,res){
    ...
    db.dog_treats.aggregate('brand', 'DISTINCT', { plain : false })
    .then(function(data){
        var result = '';
        for(var i = 0; i < data.length; i++){
            if(i != 0){
