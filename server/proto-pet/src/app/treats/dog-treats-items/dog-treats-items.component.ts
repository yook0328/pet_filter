...
@Component({
  selector: 'pet-dog-treats-items',
  templateUrl: './dog-treats-items.component.html',
  styleUrls: ['./dog-treats-items.component.css'],
  providers: [ PagerClass ]
})
export class DogTreatsItemsComponent implements OnInit {

  page_no = 0;
  Items : DogFoodItem[] = [];
  ItemsStorage : DogFoodItem[] = [];
  url = this.dataService.getURL() + '/DogTreats';
  pager: any = {};
  ...
  requestEditItem(item){
    ...
    this.dataService.setItemType('Dog');
    this.router.navigate(['/editTreatsItems']);
